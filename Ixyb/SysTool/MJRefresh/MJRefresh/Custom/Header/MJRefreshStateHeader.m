...
- (UILabel *)amountLabel {
    if (!_amountLabel) {
        [self addSubview:_amountLabel = [UILabel detailLabel]];
    }
    return _amountLabel;
}
...
- (UILabel *)lastUpdatedTimeLabel {
    if (!_lastUpdatedTimeLabel) {
        [self addSubview:_lastUpdatedTimeLabel = [UILabel detailLabel]];
    }
...
- (void)setTimeLabel:(NSString *)timeLabelStr forState:(MJRefreshState)state {
    if (timeLabelStr == nil) return;
    //    self.lastUpdatedTimeStr = timeLabelStr;
    self.amountLabel.text = timeLabelStr;
    self.isUpdatedTimeLabel = YES;
}

- (void)reloadThelastUpdatedTimeLabel {
}
...
- (void)setLastUpdatedTimeKey:(NSString *)lastUpdatedTimeKey {
    ...
    if (lastUpdatedTime) {
        ...
        // 3.显示日期
        self.lastUpdatedTimeLabel.text = [NSString stringWithFormat:@"最近更新: %@", time];
    } else {
        self.lastUpdatedTimeLabel.text = @"最近更新: 从未";
    }

    if (self.isUpdatedTimeLabel) {
...
- (void)placeSubviews {
    ...
    if (self.lastUpdatedTimeLabel.hidden) {
        ...
        if (self.isUpdatedTimeLabel) {
            ...
            // 累计交易额
            self.amountLabel.mj_x = 0;
            self.amountLabel.mj_y = self.stateLabel.mj_h;
            self.amountLabel.mj_w = self.mj_w;
            self.amountLabel.mj_h = self.mj_h * 0.5;
