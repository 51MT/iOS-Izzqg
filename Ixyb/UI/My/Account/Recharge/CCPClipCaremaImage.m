...
- (id)initWithFrame:(CGRect)frame title:(NSString *)title {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _strTitle = title;
        _transparentAreaSize = CGSizeMake(200, 300);
...
- (void)initUI {
    ...
    labelPrompt.font = TEXT_FONT_12;
    labelPrompt.text = _strTitle;
    labelPrompt.textColor = COLOR_AUXILIARY_GREY;
    labelPrompt.transform = CGAffineTransformMakeRotation(M_PI / 2);
    [self addSubview:labelPrompt];
    [labelPrompt mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.mas_centerY).offset(-23);
        make.centerX.equalTo(self.mas_centerX).offset(-((300 * ScreenScaleRatio) / 2 + 23));
    }];

    _takePhotoButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [_takePhotoButton setImage:[UIImage imageNamed:@"photograph_load"] forState:UIControlStateNormal];
    [_takePhotoButton setImage:[UIImage imageNamed:@"photograph_select"] forState:UIControlStateHighlighted];
    ...
    [cancelBut setTintColor:COLOR_COMMON_WHITE];
    cancelBut.titleLabel.font = TEXT_FONT_18;
    [cancelBut setTitle:XYBString(@"str_cancel", @"取消") forState:UIControlStateNormal];
    [cancelBut setTitleColor:COLOR_COMMON_WHITE_TRANS forState:UIControlStateHighlighted];
    ...
    [cancelBut mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_takePhotoButton.mas_centerY);
        make.left.equalTo(self.mas_left).offset(40);
    }];
}

/*!
 *  @author JiangJJ, 16-12-20 16:12:01
 *
 *  拍照
 */
- (void)takePhotoAction:(id)sender {
    [self takePhotoWithCommit:^(UIImage *image) {
        if (self.clickCaremalButton) {
            self.clickCaremalButton(image);
        }
    }];
}

/*!
 *  @author JiangJJ, 16-12-20 16:12:14
 *
 *  取消
 */
- (void)clickCancelBut:(id)sender {
    [self stopCamera];
...
- (CGRect)makeScanReaderInterrestRect {
    CGRect scanRect;
    if (MAINSCREEN_BOUNDS.size.height == 480) {
        scanRect = CGRectMake(0, 70 * ScreenScaleRatio, 300 * ScreenScaleRatio, 410 * ScreenScaleRatio);
    } else {
        scanRect = CGRectMake(0, 110 * ScreenScaleRatio, 300 * ScreenScaleRatio, 450 * ScreenScaleRatio);
    }
...
- (UIImageView *)makeScanCameraShadowViewWithRect {
    ...
    CGContextSetRGBFillColor(context, 0, 0, 0, 0.85f);
    CGContextFillRect(context, MAINSCREEN_BOUNDS);
    CGContextClearRect(context, [self makeScanReaderInterrestRect]);
    //    [self addWhiteRect:context rect:[self makeScanReaderInterrestRect]];
...
- (void)addWhiteRect:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    CGContextSetRGBStrokeColor(ctx, 123, 123, 124, 1);
