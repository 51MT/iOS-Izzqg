...
@interface DqbInvestListViewController ()<UITableViewDataSource, UITableViewDelegate, NoDataTableViewCellDelegate> {
    int count;
}
...
@implementation DqbInvestListViewController
...
- (void)initUI {
    self.navItem.title  = XYBString(@"dqb", @"定期宝");
...
- (void)initTableView {
    ...
    [self.tableView registerClass:[DqbInvestListTableViewCell class] forCellReuseIdentifier:@"investedTableViewCell"];
...
{
    NSString *urlPath = [RequestURL getRequestURL:DqbInvestOrderList param:param];
    [WebService postRequest:urlPath param:param JSONModelClass:[DqbInvestListModel class]
                    Success:^(AFHTTPRequestOperation *operation, id responseObject) {
                        [self hideLoading];
                        [self.tableView.footer resetNoMoreData];
                        self.tableView.footer.hidden = NO;
                        DqbInvestListModel * dmInvested = responseObject;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.tableView) {
        if (self.dataArray.count <= 0) {
          ...
          DqbOrderListModel * dqbOrdelist = [self.dataArray objectAtIndex:indexPath.row];
            if ([dqbOrdelist.orderState integerValue] == 3) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.tableView) {
        if (self.dataArray.count <= 0) {
            ...
            //已出借Cell
            DqbInvestListTableViewCell *cell = (DqbInvestListTableViewCell *) [tableView dequeueReusableCellWithIdentifier:@"investedTableViewCell" forIndexPath:indexPath];
            if (cell == nil) {
                cell = [[DqbInvestListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"investedTableViewCell"];
            }
            
            cell.dqbOrderList = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.tableView) {
        ...
        if (self.dataArray.count > 0) {
            DqbOrderListModel * investedProject = self.dataArray[indexPath.row];
            InvestedDetailDqbViewController *vc = [[InvestedDetailDqbViewController alloc] init];
            vc.dicDqbInfo = investedProject.toDictionary;
