...
@interface BbgInvestListViewController () <UITableViewDataSource, UITableViewDelegate, NoDataTableViewCellDelegate> {
     int count;
}
...
@implementation BbgInvestListViewController
...
- (void)initUI {
    ...
    self.navItem.title  = XYBString(@"bbg", @"步步高");
...
- (void)initTableView {
    ...
    [self.tableView registerClass:[BbgInvestListTableViewCell class] forCellReuseIdentifier:@"investedTableViewCell"];
...
{
    NSString *urlPath = [RequestURL getRequestURL:BbgInvestOrderList param:param];
    [WebService postRequest:urlPath param:param JSONModelClass:[BbgInvestListModel class]
                    Success:^(AFHTTPRequestOperation *operation, id responseObject) {
                        [self hideLoading];
                        [self.tableView.footer resetNoMoreData];
                        self.tableView.footer.hidden = NO;
                        BbgInvestListModel * dmInvested = responseObject;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.tableView) {
        if (self.dataArray.count <= 0) {
            ...
            //已出借Cell
            BbgInvestListTableViewCell *cell = (BbgInvestListTableViewCell *) [tableView dequeueReusableCellWithIdentifier:@"investedTableViewCell" forIndexPath:indexPath];
            if (cell == nil) {
                cell = [[BbgInvestListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"investedTableViewCell"];
            }
       
            cell.bbgOrderList = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.tableView) {
        ...
        if (self.dataArray.count > 0) {
            BbgOrderListModel * investedProject = self.dataArray[indexPath.row];
            InvestedDetailBbgViewController *vc = [[InvestedDetailBbgViewController alloc] init];
            vc.dicBbgInfo = investedProject.toDictionary;
