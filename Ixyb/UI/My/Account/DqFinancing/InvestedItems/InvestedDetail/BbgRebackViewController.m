...
#import "InvestedItemsViewController.h"
...
- (void)initUI {
    self.navigationItem.title = XYBString(@"str_bbg_back", @"步步高本金赎回");
    ...
    viewContent.layer.shadowColor = COLOR_REDEEM.CGColor;
    viewContent.layer.shadowOffset = CGSizeMake(0,-4);
    viewContent.layer.shadowOpacity = 0.69;
    ...
    [viewContent addSubview:viewHead];
    [viewHead mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@(Cell_Height));
        make.left.top.right.equalTo(viewContent);
    }];
    
    UIImageView * imageView = [[UIImageView alloc] initWithFrame:CGRectZero];
    imageView.image = [UIImage imageNamed:@"redeemable"];
    ...
    [allInvestBtn setTitleColor:COLOR_HAVECAST_GREEN forState:UIControlStateNormal];
    ...
    [viewHead addSubview:allInvestBtn];
    ...
    [viewContent addSubview:tip2Label];
    tip2Label.text = [NSString stringWithFormat:XYBString(@"str_to_account_date", @"预计到账日期:%@"),@"2017-09-15"];
    ...
    [tip2Label mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(@Margin_Length);
        make.top.equalTo(viewContent.mas_bottom).offset(12);
    }];
    ...
    [scrollView addSubview:rebackButton];
    
    [rebackButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(@(Margin_Left));
        make.right.equalTo(@(-Margin_Right));
        make.height.equalTo(@(Cell_Height));
        make.top.equalTo(viewContent.mas_bottom).offset(Cell_Height);
    }];
    ...
    [agreementButton setTitleColor:COLOR_HAVECAST_GREEN forState:UIControlStateNormal];
...
//                NSString *subTitle2 = [NSString stringWithFormat:XYBString(@"str_expect_in_account_some", @"预计到账%@元"), rebackAmount];
                ...
                bbgReback.applyDate = currentFormtTime;
                bbgReback.moneyStr = [NSString stringWithFormat:@"%@",self.moneyTextField.text];
                bbgReback.estimateDate = refundFormtTime;
