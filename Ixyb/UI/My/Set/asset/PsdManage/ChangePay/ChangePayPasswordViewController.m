...
@interface ChangePayPasswordViewController () <UITextFieldDelegate> {
    MBProgressHUD *hud;
}
...
- (void)createMainView {
    ...
    [self.phoneLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(phoneLab.mas_right).offset(-8); //缩进了同冒号间的距离
...
- (IBAction)clcikTheCodeButton:(id)sender {
    ...
    [self requestGetVerifyCodeWithDictionary:@{
        @"mobilePhone" : [UserDefaultsUtil getUser].tel,
        @"businessType" : [NSNumber numberWithInt:4],
    ...
    dispatch_source_set_event_handler(timer, ^{
        if (timeout <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                _codeButton.titleLabel.font = TEXT_FONT_12;
                _codeButton.isEnabled = NO;
...
/*****************************校验手机验证码接口**********************************/
- (void)callVerifyWebService:(NSDictionary *)dictionary {
    _nextButton.enabled = NO;
    NSString *requestURL = [RequestURL getRequestURL:UserCodeVerifyURL param:dictionary];
    [self showDataLoading];
    [WebService postRequest:requestURL param:dictionary JSONModelClass:[ResponseModel class] Success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [self hideLoading];
        ResponseModel *responseModel = responseObject;
        if (responseModel.resultCode == 1) {
            RevisePaywordViewController *revisePaywordViewController = [[RevisePaywordViewController alloc] init];
            revisePaywordViewController.codeStr = _codeTextField.text;
            [self.navigationController pushViewController:revisePaywordViewController animated:YES];
        }
