...
- (void)initUI {
    ...
    NSString *navigationItemTitle = XYBString(@"str_person_bj_address_gl", @"编辑收货地址");
    if ([UserDefaultsUtil getUser]) {
        User *user = [UserDefaultsUtil getUser];
        if (!user.isHaveAddr) {
            navigationItemTitle = XYBString(@"str_person_set_address_gl", @"设置收货地址");
        }
    }
    ...
    [button setTitle:XYBString(@"str_person_save", @"保存") forState:UIControlStateNormal];
    ...
    self.recipientsLabel.text = XYBString(@"str_person_consignee_name", @"收货人姓名");
    ...
    self.recipientsTextField.placeholder = XYBString(@"str_person_please_input_receiver_name", @"请输入收货人姓名");
    ...
    self.mobilePhoneLabel.text = XYBString(@"str_person_phone_number", @"手机号码");
    ...
    self.mobilePhoneTextField.placeholder = XYBString(@"str_person_please_input_phone_number", @"请输入手机号码");
    ...
    self.areaLabel.text = XYBString(@"str_person_region", @"所在区域");
    ...
    self.detailLabel.text = XYBString(@"str_person_detailed_address", @"详细地址");
    ...
    self.detailTextField.placeholder = XYBString(@"str_please_input_detail_location", @"请输入详细地址");
...
- (void)clickTheRightBtn:(id)sender {
    ...
    if (recipients.length == 0) {
        [HUD showPromptViewWithToShowStr:XYBString(@"str_person_please_input_receiver_name", @"请输入收货人姓名") autoHide:YES afterDelay:1.0 userInteractionEnabled:YES];
        return;
    }

    if (recipients.length > 10) {
        [HUD showPromptViewWithToShowStr:XYBString(@"str_reveiver_name_most_ten", @"收货人姓名最多10个字符") autoHide:YES afterDelay:1.0 userInteractionEnabled:YES];
        return;
    }
    ...
    if (self.selectAreaLabel.text.length == 0 || [self.selectAreaLabel.text isEqualToString:@""]) {
        [HUD showPromptViewWithToShowStr:XYBString(@"str_person_please_input_region", @"请选择所在区域") autoHide:YES afterDelay:1.0 userInteractionEnabled:YES];
        return;
    }

    if (detail.length == 0) {
        [HUD showPromptViewWithToShowStr:XYBString(@"str_please_input_detail_location", @"请输入详细地址") autoHide:YES afterDelay:1.0 userInteractionEnabled:YES];
        return;
    }

    if (detail.length > 30) {
        [HUD showPromptViewWithToShowStr:XYBString(@"str_location_detail_most_thirty", @"详细地址最多30个字符") autoHide:YES afterDelay:1.0 userInteractionEnabled:YES];
        return;
    }
