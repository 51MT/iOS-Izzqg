...
@interface DqbInvestViewController () <RTLabelDelegate> {
...
@implementation DqbInvestViewController
...
- (void)creatTheMainScrollView {
    mainScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth, MainScreenHeight - 40)];
...
-(void)createTheTopUI {
    ...
    titleLab.text = XYBString(@"str_common_dqb", @"定期宝");
    ...
    if (_ccInfo) {
        ...
        titleLab.text = [NSString stringWithFormat:XYBString(@"str_financing_dqb_investBid", @"定期宝-%@"),_ccInfo.typeStr];
        
        //历史年化结算利率
        NSString *rateStr = [Utility rateReplaceTheNumberForNSNumberFormatter:[NSString stringWithFormat:@"%.2f", [_ccInfo.rate doubleValue] * 100]];
        rateStr = [rateStr stringByAppendingString:XYBString(@"str_financing_percentSymbol", @"%")];
        rateLab.attributedText = [self getAttributeStrWithString:rateStr];
        
        //期限
        standardLab.text =  _ccInfo.periodsStr;
        
        //剩余可投
        if ([_ccInfo.restAmount doubleValue] <= 0) {
            restLab.text = [NSString stringWithFormat:XYBString(@"str_financing_restMoneyBlow", @"剩余可投:%@元"),@"0.00"];
        }
        
        if ([_ccInfo.restAmount doubleValue] > 0) {
            restLab.text = [NSString stringWithFormat:XYBString(@"str_financing_restMoneyBlow", @"剩余可投:%@元"),[Utility replaceTheNumberForNSNumberFormatter:_ccInfo.restAmount]];
        }
    }
}
...
- (void)creatTheInvestDetailView {
    ...
    //定期宝“日新月异、周周盈”进入出借页面不可以使用优惠券，所以在此判断，如果是日新月异，就直接影藏使用优惠券的UI
    if ([self.ccInfo.type isEqualToString:@"RXYY"] || [self.ccInfo.type isEqualToString:@"ZZY"]) {
        couponsBtn.hidden = YES;
        [couponsBtn mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)creatTheInvestBtnView {
    ...
    if ([_ccInfo.type isEqualToString:@"ZZY"]) {
        protocolStr = [NSString stringWithFormat:XYBString(@"str_financing_djqrcjjbsydbty", @"点击“确认出借”，即表示阅读并同意<font color='#4385f5' ><u color=clear><a href='%@'>%@</a></u></font>和<font color='#4385f5' ><u color=clear><a href='fxjss'>《风险警示书》</a></u></font>"),@"zzyfwxy", XYBString(@"str_financing_zzyAgreement", @"《周周盈服务协议》")];
        
    }else{
       protocolStr = [NSString stringWithFormat:XYBString(@"str_financing_djqrcjjbsydbty", @"点击“确认出借”，即表示阅读并同意<font color='#4385f5' ><u color=clear><a href='%@'>%@</a></u></font>和<font color='#4385f5' ><u color=clear><a href='fxjss'>《风险警示书》</a></u></font>"),@"dqbfwxy", XYBString(@"str_financing_dqbAgreement", @"《定期宝服务协议》")];
    }
    ...
    //1. 定期宝的剩余可投为0，出借按钮置灰 2. 若定期宝为预定标，则全投按钮置灰，投资金额为预约金额，且不可编辑
    if (_ccInfo) {
        if ([_ccInfo.restAmount doubleValue] <= 0) {
            _nextButton.isColorEnabled = NO;
        ...
        if ([_ccInfo.type isEqualToString:@"ZZY"] && [_ccInfo.restInvestCount doubleValue] == 0) {
            _nextButton.isColorEnabled = NO;
            dustedMoneyTextField.enabled = NO;
            allInvestBtn.isEnabled = NO;
        }
        ...
        if (_isBookBid == YES) {
            dustedMoneyTextField.text = [Utility notRounding:[_ccInfo.restAmount doubleValue] afterPoint:0];
            dustedMoneyTextField.enabled = NO;
            allInvestBtn.isEnabled = NO;
        }
    }
...
- (void)rtLabel:(id)rtLabel didSelectLinkWithURL:(NSURL *)url {
    NSString *urlStr;
    if ([url.description isEqualToString:@"zzyfwxy"]) { // 周周盈服务协议
        urlStr = [RequestURL getNodeJsH5URL:App_Zzy_Protocol_URL withIsSign:NO];
        
    }else if ([url.description isEqualToString:@"dqbfwxy"]) { // 定期宝服务协议
        urlStr = [RequestURL getNodeJsH5URL:App_CC_Protocol_URL withIsSign:NO];
...
- (void)clickTheAllInvestBtn {
    ...
    // fromTag必须传，定期宝：“策诚”；信投保：“信投宝”
    dustedMoneyTextField.text = [CheckTheInvestMoney allInvestMoneyPreInvest:mainDic fromTag:XYBString(@"str_common_cc", @"策诚") prouduct:nil ccProuduct:self.ccInfo];
    [self doaction];
}
...
- (void)clickTheNextButton:(id)sender {
    ...
    if ([user.isIdentityAuth boolValue]) {
        ...
        BOOL isScuess = [CheckTheInvestMoney checkTheInvestMoney:dustedMoneyTextField.text preInvest:mainDic fromTag:XYBString(@"str_common_cc", @"策诚") prouduct:nil ccProuduct:self.ccInfo];
        
        //添加优惠券判断：若用户有优惠券，但用户未使用时进行提示 (前提是投资金额检验通过，有可用优惠券，并且未选择优惠券,且未弹窗提示过)
        if (isScuess == YES && _couponsAlert == NO && _usableCard == YES  && [StrUtil isEmptyString:_couponsID] && ![couponsDetailLab.text isEqualToString:@"未使用"] && ![self.ccInfo.type isEqualToString:@"ZZY"]) {
...
- (void)payTheFinancing:(NSString *)moneyStr {
    ...
    User *user = [UserDefaultsUtil getUser];
    __weak DqbInvestViewController *investVC = self;
...
#pragma mark - 定期宝出借前接口

- (void)setRequestForDetail {
    
    productIdStr = _ccInfo.ccId;
...
- (void)requestDqbInvestWebServiceWithParam:(NSDictionary *)params {
    ...
    NSString *requestURL = [RequestURL getRequestURL:DqbInvestURL param:params];
    ...
    [WebService postRequest:requestURL param:params JSONModelClass:[DqbAndXtbInvestResponseModel class]
                    ...
                    Success:^(AFHTTPRequestOperation *operation, id responseObject) {
                        ...
                        incomeStatusViewController.fromTag = XYBString(@"str_common_cc", @"策诚");
                        incomeStatusViewController.type = _ccInfo.type;
