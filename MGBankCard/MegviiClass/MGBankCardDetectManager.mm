        ...
        @synchronized(self){
            if (_checkFinish == NO) {
                ...
                if (cardResult.confidence > KBsetConfidence) {
                    [self checkFinish:cardResult bankCardImage:tempImage];
                    return;
                }

                if ([self startFirstProgerm:cardResult bankCardImage:tempImage] == NO) {
                    [self startSecondProgerm:cardResult bankCardImage:tempImage];
                }
            }
        }
    }
