//
//  FinancialCustomerViewController.m
//  Ixyb
//
...
@interface FinancialCustomerViewController () <UIWebViewDelegate>

@end

@implementation FinancialCustomerViewController
...
- (void)createNav {
    ...
    self.navItem.title = XYBString(@"str_sidebar_lc_customer", @"出借客服");
...
- (void)initUI {
    ...
    lbaleKfRihhtPhone.text = @"400-070-7663";
    ...
    lbaleRihhtNumber.text = XYBString(@"str_loan_xyd", @"宝妹在线");
    ...
    UIButton *butProblem = [UIButton buttonWithType:UIButtonTypeCustom];
    [butProblem setTitle:XYBString(@"str_xyb_tips", @"常见问题及产品介绍") forState:UIControlStateNormal];
    [butProblem setTitleColor:COLOR_MAIN forState:UIControlStateNormal];
    [butProblem setTitleColor:COLOR_HIGHTBULE_BUTTON forState:UIControlStateHighlighted];
    butProblem.titleLabel.font = TEXT_FONT_12;
    [butProblem addTarget:self action:@selector(clickProblem:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:butProblem];
    [butProblem mas_makeConstraints:^(MASConstraintMaker *make) {
 ...
 *  @return
 */
- (void)clickProblem:(UIButton *)but {
    NSString *urlStr = [RequestURL getNodeJsH5URL:App_Faq_Index_URL withIsSign:NO];
    NSString *titleStr = XYBString(@"str_xyb_tips", @"常见问题及产品介绍");
...
- (void)kfPhoneControl:(XYButton *)but {
    UIAlertView *alertview = [[UIAlertView alloc] initWithTitle:XYBString(@"str_xyb_lc_tellphone", @"拨打电话: 400-070-7663")
...
- (void)wxPublicNumberControl:(XYButton *)but {
    ...
    pasteboard.string = @"宝妹在线";
    UIAlertView *alertview = [[UIAlertView alloc] initWithTitle:XYBString(@"str_xyb_alert", @"提示")
                                                        message:XYBString(@"str_xyb_wx_tip", @"公众号已复制到粘贴板，您可以微信-通讯录-搜索框粘贴“宝妹在线”公众号，点击关注即可。")
                                                       delegate:self
                                              cancelButtonTitle:XYBString(@"str_cancel", @"取消")
