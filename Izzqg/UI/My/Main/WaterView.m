...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _waveCycle=1.0;
        increase=NO;
        _waterColor= [[UIColor whiteColor] colorWithAlphaComponent:0.1f];
        _waterLineY= 10.f;
...
- (void)drawRect:(CGRect)rect
{
    ...
    //后波浪绘制
    CGContextSetFillColorWithColor(context, [[UIColor whiteColor] colorWithAlphaComponent:0.1f].CGColor);
    ...
    //后波浪反色绘制
    CGPathAddLineToPoint(backReversePath, nil, rect.size.width, rect.size.height);
    ...
    CGPathAddLineToPoint(frontPath, nil, rect.size.width, rect.size.height);
    ...
    //前波浪反色绘制
    CGPathAddLineToPoint(frontReversePath, nil, rect.size.width, rect.size.height);
