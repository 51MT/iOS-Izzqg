...
@property (nonatomic) BOOL closeTag;
@end
...
- (void)setDefaultParameters {
    ...
    _axisColor = [UIColor colorWithWhite:0.7 alpha:0.5];
    ...
    // Labels attributes
    _indexLabelBackgroundColor = COLOR_COMMON_CLEAR;
    _indexLabelTextColor = [UIColor grayColor];
    _indexLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:8];

    _valueLabelBackgroundColor = [UIColor colorWithWhite:1 alpha:0.75];
    _valueLabelTextColor = [UIColor grayColor];
    _valueLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:8];
...
- (void)setChartData:(NSArray *)chartData {
    ...
    if (_labelForIndex) {
        for (int i = 0; i < _labelIndexArr.count; i++) {
            UILabel *label = [self createLabelForIndex:i];

            if (label && label.tag == 200 + i) {
                [label removeFromSuperview];
                [self addSubview:label];
            }
        }
...
//每行平均高度
- (float)averageTheHeight {
    float hight = (self.frame.size.height - _margin * 2) / _verticalGridStep;
    return hight;
}

- (void)setLabelVauleArr:(NSArray *)labelVauleArr {
    _labelVauleArr = labelVauleArr;
    NSString *maxVaule = labelVauleArr[0];
    for (NSString *vaule in labelVauleArr) {
        if ([maxVaule floatValue] < [vaule floatValue]) {
            _leftVauleMax = [vaule floatValue];
        }
    }
}

- (void)setLabelIndexArr:(NSArray *)labelIndexArr {
    _labelIndexArr = labelIndexArr;
}
...
- (UILabel *)createLabelForValue:(NSUInteger)index {
    ...
    if (index < _labelVauleArr.count) {
        text = [_labelVauleArr objectAtIndex:index];
    }
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(p.x - xOffset, p.y - 6, width + 2, 12)];
    label.text = text;
...
- (UILabel *)createLabelForIndex:(NSUInteger)index {
    ...
    NSInteger q = (int) _labelIndexArr.count / _horizontalGridStep;
    NSInteger itemIndex = q * index;

    if (itemIndex >= _labelIndexArr.count) {
        itemIndex = _labelIndexArr.count - 1;
    }

    NSString *text = _labelForIndex(itemIndex);
    if (index < _labelIndexArr.count) {
        text = [_labelIndexArr objectAtIndex:index];
    }
    ...
    CGPoint p = CGPointMake(_margin + index * (_axisWidth / _labelIndexArr.count) * scale, _axisHeight + _margin);
    CGRect rect = CGRectMake(_margin, p.y + 2, self.frame.size.width - _margin * 2 - 4.0f, 14);
    float width = [text boundingRectWithSize:rect.size
                                     options:NSStringDrawingUsesLineFragmentOrigin
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(p.x - 10.0f, p.y + 2, width + 2, 14)];
    label.text = text;
...
- (void)drawRect:(CGRect)rect {
    if (_data.count > 0) {
        [self drawGrid];
        [self drawTheLastPoint:(int) _data.count - 1];
    }
}
...
- (void)drawGrid {
    ...
    if (_drawInnerGrid) {
        for (int i = 0; i < _labelIndexArr.count; i++) {
            ...
            CGPoint point = CGPointMake((1 + i) * _axisWidth / _labelIndexArr.count * scale + _margin, _margin);
            CGContextSetStrokeColorWithColor(ctx, [_axisColor CGColor]);
            CGContextSetLineWidth(ctx, _axisLineWidth);
...
- (void)strokeChart {
    ...
    _closeTag = NO;
    UIBezierPath *noPath = [self getLinePath:0 withSmoothing:_bezierSmoothing close:_closeTag];
    UIBezierPath *path = [self getLinePath:scale withSmoothing:_bezierSmoothing close:_closeTag];
    _closeTag = YES;
    ...
    gradientLayer.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y + minBound * scale, self.bounds.size.width, self.bounds.size.height);
    gradientLayer.colors = @[ (__bridge id) COLOR_ORANGE.CGColor, (__bridge id) COLOR_HQB_FSLINECHAERT.CGColor ];
...
- (CGFloat)horizontalScale {
    CGFloat scale = 1.0f;
    if (_data.count > 1) {
        scale = (CGFloat)(_labelIndexArr.count) / (CGFloat)(_labelIndexArr.count - 1);
    }
...
- (CGPoint)getPointForIndex:(NSUInteger)idx withScale:(CGFloat)scale {
    ...
    float hight = [self averageTheHeight];
    if (_data.count < 2) {
        return CGPointMake(_margin, (_leftVauleMax - [number floatValue]) * hight);
    } else {
        float pointHight = (_leftVauleMax - [number floatValue]) * hight;
        if (pointHight <= 0) {
            pointHight = _margin;
        }
        if (!_bezierSmoothing) {
            if (!_closeTag) {
                if (idx == _data.count - 1) {
                    return CGPointMake(_margin + idx * (_axisWidth / (_data.count - 1)) - 3, pointHight);
                }
            }
        }

        return CGPointMake(_margin + idx * (_axisWidth / (_data.count - 1)), pointHight);
    }
}
...
//画结局圆圈和label
- (void)drawTheLastPoint:(int)i {
    if (i == _data.count - 1) {
        CGContextRef ctx = UIGraphicsGetCurrentContext();
        //边框圆
        CGContextSetRGBStrokeColor(ctx, 221 / 255.f, 106 / 255.f, 84 / 255.f, 1.0); //画笔线的颜色
        CGContextSetLineWidth(ctx, 2.0);                                            //线的宽度
        // x,y为圆点坐标，radius半径，startAngle为开始的弧度，endAngle为 结束的弧度，clockwise 0为顺时针，1为逆时针
        CGFloat minBound = [self minVerticalBound];
        CGFloat maxBound = [self maxVerticalBound];
        CGFloat scale1 = _axisHeight / (maxBound - minBound);
        //    NSNumber *number = _data[_data.count-1];
        CGPoint point = [self getPointForIndex:_data.count - 1 withScale:scale1];
        //    CGPointMake(_margin + 6 * (_axisWidth / (_data.count - 1)), _axisHeight + _margin - [number floatValue] * scale1);
        float pontY = 0;
        if (point.y == 0) {
            pontY = 5;
        } else {
        ...
        if (_data.count < 2) {
            CGContextAddArc(ctx, _margin, pontY, 3, 0, 2 * 3.14, 0); //添加一个圆
        } else {
            CGContextAddArc(ctx, _axisWidth + _margin, pontY, 3, 0, 2 * 3.14, 0); //添加一个圆
        }

        CGContextDrawPath(ctx, kCGPathStroke); //绘制路径

        UILabel *lab = [[UILabel alloc] init];
        NSString *labStr = [Utility stringrangeStr:[NSString stringWithFormat:@"%.2f", [_data[i] doubleValue]]];
        lab.text = labStr;
        lab.font = XTB_ADD_RATE_FONT;
        lab.layer.masksToBounds = YES;
        lab.layer.cornerRadius = Corner_Radius_3;
        lab.textAlignment = NSTextAlignmentCenter;
        lab.textColor = COLOR_COMMON_WHITE;
        lab.backgroundColor = COLOR_ORANGE;
        if (_data.count < 2) {
            lab.frame = CGRectMake(_margin, point.y - 25, 20, 13);
        } else {
            lab.frame = CGRectMake(_axisWidth + _margin - 20, point.y - 25, 20, 13);
        }
        [self addSubview:lab];
    }
}
